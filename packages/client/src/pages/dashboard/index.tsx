...
export function DashboardPage() {
  ...
  return (
    <div>
      ...
      {/* 员工网格 */}
      <Title level={5} style={{ color: '#bfbfbf', marginBottom: 12 }}>员工状态</Title>
      {agents.length === 0 ? (
        <Card style={{ background: '#1f1f1f', textAlign: 'center', padding: 40 }}>
          <div style={{ color: '#595959', fontSize: 16 }}>
            还没有员工，去<a href="/market" style={{ color: '#1890ff' }}>招聘市场</a>招募第一个吧
          </div>
        </Card>
      ) : (
        <Row gutter={[12, 12]}>
              ...
              <Col key={agent.id} xs={12} sm={8} md={6} lg={4}>
                ...
                >
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontWeight: 600, color: '#e8e8e8', marginBottom: 2 }}>{agent.name}</div>
                    <div style={{ fontSize: 12, color: '#8c8c8c', marginBottom: 8 }}>{roleInfo.label}</div>
                    <div style={{
